            ...
            <div class="space-y-8 mb-8">
                <div v-for="item in cart.items" :key="item.productId"
                    ...
                    <img :src="item.image" :alt="item.name"
                        class="w-28 h-28 object-contain rounded-lg bg-argan-light" />
                    <div class="flex-1">
                        ...
                        <div class="flex items-center gap-2">
                            <button @click="decrease(item)"
                                class="px-3 py-1 text-xl text-argan-gold hover:text-argan-dark"
                                :disabled="item.quantity <= 1">-</button>
                            <span class="font-medium">{{ item.quantity }}</span>
                            <button @click="increase(item)"
                                class="px-3 py-1 text-xl text-argan-gold hover:text-argan-dark">+</button>
                        </div>
                    </div>
                    <div class="flex flex-col items-end">
                        <button @click="remove(item)"
                            class="text-red-400 hover:text-red-700 text-sm mt-2 flex items-center gap-2">
